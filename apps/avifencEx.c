...
//
// avifencEx: avifenc + --filelist support.
// This file is a copy of avifenc.c (as of upstream commit d0605c98) with the
// --filelist option added. Search for "avifencEx addition" for the diff.
// If this file drifts far from avifenc.c again, re-copy avifenc.c and
// re-apply the --filelist changes instead of hand-patching individual options.
...
typedef struct avifInputFile
{
    ...
    avifBool fromFilelist; // avifencEx addition: if AVIF_TRUE, filename is heap-allocated and must be freed
} avifInputFile;
...
static void syntaxLong(void)
{
    ...
    printf("    --filelist FILENAME               : Read input PNG/JPEG/y4m paths from a text file (one per line, optional \",DURATION\" suffix for per-frame duration). "
           "Mutually exclusive with positional input files and --stdin.\n");
...
// avifencEx addition: load input filenames from a text file.
// Format: one path per line; optional ",N" suffix sets this entry's frame duration (in timescale
// ticks). Lines starting with '#' and blank lines are ignored. A comma is used as the duration
// separator (instead of ':') to avoid any ambiguity with Windows drive letters (e.g. "C:\...").
// Appends loaded entries to input->files via avifInputAdd(). Returns AVIF_FALSE on error.
static avifBool loadFilelist(const char * listFilename, avifInput * input, uint64_t defaultDuration, avifInputFileSettings * pendingSettings)
{
    ...
    // First pass: count valid lines so input->files can be grown once.
    int count = 0;
    char line[4096];
    ...
    if (count == 0) {
        fclose(f);
        fprintf(stderr, "ERROR: Filelist contains no entries: %s\n", listFilename);
        return AVIF_FALSE;
    }

    avifInputFile * newFiles = realloc(input->files, sizeof(avifInputFile) * (size_t)(input->filesCount + count));
    if (!newFiles) {
        fclose(f);
        fprintf(stderr, "ERROR: Out of memory\n");
        return AVIF_FALSE;
    ...
    rewind(f);
    int added = 0;
    while (fgets(line, sizeof(line), f) && added < count) {
        char * p = line;
        while (*p == ' ' || *p == '\t') {
            ++p;
        }
        ...
        // An optional ",DURATION" suffix overrides this entry's frame duration.
        char * lastComma = strrchr(p, ',');
        uint64_t duration = 0; // 0 = use the global default duration
        if (lastComma) {
            const char * afterComma = lastComma + 1;
            avifBool allDigits = (*afterComma != '\0');
            for (const char * c = afterComma; *c; ++c) {
                if (!isdigit((unsigned char)*c)) {
                    allDigits = AVIF_FALSE;
                    break;
                }
            }
            if (allDigits) {
                duration = (uint64_t)atoll(afterComma);
                *lastComma = '\0';
            }
        }

        char * path = avifStrdup(p);
        if (!path) {
            fclose(f);
            fprintf(stderr, "ERROR: Out of memory\n");
            return AVIF_FALSE;
        ...
        avifInputAdd(input, path, duration ? duration : defaultDuration, pendingSettings);
        input->files[input->filesCount - 1].fromFilelist = AVIF_TRUE;
        ++added;
    }
    fclose(f);
    return AVIF_TRUE;
...
int main(int argc, char * argv[])
{
    ...
    // avifencEx addition
    const char * filelistFilename = NULL;
    avifBool filelistUsed = AVIF_FALSE;
    ...
    while (argIndex < argc) {
        ...
        if (!strcmp(arg, "--")) {
            ...
            while (argIndex < argc) {
                arg = argv[argIndex];
                if (filelistUsed) {
                    fprintf(stderr, "ERROR: --filelist and positional input files are mutually exclusive\n");
                    goto cleanup;
                }
            ...
            if (filelistUsed) {
                fprintf(stderr, "ERROR: --filelist and --stdin are mutually exclusive\n");
                goto cleanup;
            }
            ...
            // avifencEx addition: load input files from a text file
            NEXTARG();
            if (input.filesCount > 0) {
                fprintf(stderr, "ERROR: --filelist and other input files (including --stdin) are mutually exclusive\n");
                goto cleanup;
            }
            filelistFilename = arg;
            filelistUsed = AVIF_TRUE;
    ...
    // avifencEx addition: load filelist after all args are parsed so --timescale/--duration are set.
    if (filelistFilename) {
        if (!loadFilelist(filelistFilename, &input, settings.outputTiming.duration, &pendingSettings)) {
            goto cleanup;
        }
        printf("Loaded filelist: %s (%d frames)\n", filelistFilename, input.filesCount);
    }
...
cleanup:
    ...
    while (input.filesCount) {
        ...
        // avifencEx addition: free heap-allocated filenames loaded from --filelist
        if (file->fromFilelist) {
